...
namespace Aftermath {
    ...
    public sealed class ChangeSet {
        ...
        public ReadOnlyCollection<ChangeSetEntry> ChangeSetEntries {
            get {
                return _changeSetEntries.ToList().AsReadOnly();
            }
        }
        ...
        public bool HasError {
            get {
                return _changeSetEntries.Any(op => op.HasConflict || (op.ValidationErrors != null && op.ValidationErrors.Any()));
            }
        }
        ...
        public TEntity GetOriginal<TEntity>(TEntity clientEntity) where TEntity : class {
            ...
            var entry = _changeSetEntries.FirstOrDefault(p => ReferenceEquals(p.Entity, clientEntity));
            if (entry == null)
        ...
        private static void ValidateChangeSetEntries(IEnumerable<ChangeSetEntry> changeSetEntries) {
            ...
            var entitySet = new HashSet<object>();
            foreach (var entry in changeSetEntries) {
        ...
        private static void ValidateAssociationMap(Type entityType, ICollection<int> idSet, IEnumerable<KeyValuePair<string, int[]>> associationMap) {
            var properties = TypeDescriptor.GetProperties(entityType);

            foreach (var associationItem in associationMap) {
                // ensure that the member is an association member
                var associationMemberName = associationItem.Key;
                var associationMember = properties[associationMemberName];
                if (associationMember == null || associationMember.Attributes[typeof (AssociationAttribute)] == null) {
                    throw Error.InvalidOperation(
                        Resource.InvalidChangeSet,
                ...
                foreach (var id in associationItem.Value.Where(id => !idSet.Contains(id)))
                    throw Error.InvalidOperation(Resource.InvalidChangeSet, String.Format(CultureInfo.CurrentCulture, Resource.InvalidChangeSet_AssociatedIdNotInChangeset, id, entityType, associationMemberName));
            }
        }
        ...
        internal void SetEntityAssociations() {
            ...
            var entityIdMap = _changeSetEntries.ToDictionary(
                p => p.Id,
                p => new {
                    p.Entity,
                    p.OriginalEntity
                });
            foreach (var changeSetEntry in _changeSetEntries) {
                var entity = entityIdMap[changeSetEntry.Id].Entity;
                if (changeSetEntry.Entity != entity)
                    changeSetEntry.Entity = entity;

                var original = entityIdMap[changeSetEntry.Id].OriginalEntity;
            ...
            foreach (var entityGroup in _changeSetEntries.Where(p => (p.Associations != null && p.Associations.Count > 0) || (p.OriginalAssociations != null && p.OriginalAssociations.Count > 0)).GroupBy(p => p.Entity.GetType())) {
                var associationMemberMap = TypeDescriptor.GetProperties(entityGroup.Key).Cast<PropertyDescriptor>().Where(p => p.Attributes[typeof (AssociationAttribute)] != null).ToDictionary(p => p.Name);
                foreach (var changeSetEntry in entityGroup.Where(changeSetEntry => !visited.Contains(changeSetEntry.Id))) {
                    ...
                    if (changeSetEntry.Associations != null) {
                        foreach (var associationItem in changeSetEntry.Associations) {
                            var assocMember = associationMemberMap[associationItem.Key];
                            var children = associationItem.Value.Select(p => entityIdMap[p].Entity);
        ...
        internal bool Validate(HttpActionContext actionContext) {
            ...
            var success = true;
            var entriesToValidate = ChangeSetEntries.Where(
        ...
        private static void SetAssociationMember(object entity, PropertyDescriptor associationProperty, IEnumerable<object> associatedEntities) {
            ...
            var associationValue = associationProperty.GetValue(entity);
            if (typeof (IEnumerable).IsAssignableFrom(associationProperty.PropertyType)) {
                ...
                var list = associationValue as IList;
                IEnumerable<object> associationSequence = null;
                MethodInfo addMethod = null;
                if (list == null) {
                    // not an IList, so we have to use reflection
                    var associatedEntityType = TypeUtility.GetElementType(associationValue.GetType());
                    addMethod = associationValue.GetType().GetMethod(
                        "Add",
                ...
                var associatedEntity = associatedEntities.Single();
                var currentValue = associationProperty.GetValue(entity);
                if (!Equals(currentValue, associatedEntity))
