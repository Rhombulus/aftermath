...
namespace Aftermath {
    ...
    public abstract class DbDataController<TContext> : DataController
        ...
        /// <summary>
        ///   Gets the <see cref="System.Data.Entity.Core.Objects.ObjectContext" /> used for retrieving store values
        /// </summary>
        ...
        ///   <remarks>
        ///     If the submit fails due to concurrency conflicts <see cref="ResolveConflicts" /> will be called.
        ...
        private bool InvokeSaveChanges(bool retryOnConflict) {
            try {
                ...
                var operationConflictMap = new Dictionary<DbEntityEntry, ChangeSetEntry>();
                foreach (var conflict in ex.Entries) {
                    var entry =
                        ChangeSet.ChangeSetEntries.SingleOrDefault(p => ReferenceEquals(p.Entity, conflict.Entity));
                    if (entry == null) {
        ...
        private void SetChangeSetConflicts(Dictionary<DbEntityEntry, ChangeSetEntry> operationConflictMap) {
            ...
            foreach (var conflictEntry in operationConflictMap) {
                var entityEntry = conflictEntry.Key;
                var stateEntry = objectStateManager.GetObjectStateEntry(entityEntry.Entity);
                ...
                var operationInConflict = conflictEntry.Value;
                var refreshEntityKey = RefreshContext.CreateEntityKey(stateEntry.EntitySet.Name, stateEntry.Entity);
                object storeValue;
                ...
                var isDeleted = (operationInConflict.StoreEntity == null);
                if (isDeleted)
                    ...
                    var propDescriptors = TypeDescriptor.GetProperties(operationInConflict.Entity.GetType());
                    var membersInConflict = new List<string>();
                    for (var i = 0; i < stateEntry.OriginalValues.FieldCount; i++) {
                        var originalValue = stateEntry.OriginalValues.GetValue(i);
                        if (originalValue is DBNull)
                            originalValue = null;

                        var propertyName = stateEntry.OriginalValues.GetName(i);
                        var propertyDescriptor = propDescriptors[propertyName];
                        ...
                        if (!Equals(originalValue, propertyDescriptor.GetValue(operationInConflict.StoreEntity)))
                            membersInConflict.Add(propertyDescriptor.Name);
                    }
        ...
        /// <summary>
        ///   Insert an entity into the <see cref="DbContext" />, ensuring its <see cref="System.Data.Entity.EntityState" /> is <see
        ///    cref="System.Data.Entity.EntityState.Added" />
        /// </summary>
        ...
        /// <summary>
        ///   Delete an entity from the <see cref="DbContext" />, ensuring that its <see cref="System.Data.Entity.EntityState" /> is <see
        ///    cref="System.Data.Entity.EntityState.Deleted" />
        /// </summary>
